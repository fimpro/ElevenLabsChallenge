...
class User:
    ...
    def do_api_request(self):
        ...
        distance = geodesic(self.prev_request_location, self.get_request_location()).meters
        if time_since_last_request < MIN_TIME or distance < MIN_DISTANCE:
            if PRINT_OUTPUTS:
                print(f"Rejected doing api request: {time_since_last_request:.2f}s passed (min {MIN_TIME}s), {distance}m travelled (min {MIN_DISTANCE}m)")
            return False
...
async def create_token(req: CreateTokenRequest):
    ...
    users[token] = User(', '.join(req.preferences) if len(req.preferences) > 0 else 'none', req.emotions.lower(), req.voice.lower())
...
With that in mind, choose the best location from these or choose that none of them are really worth seeing:""" 
    ...
    chat.message(prompt)
    str_chosen_id = chat.message("Now, for automatic record, write ONLY the number of the place that you have chosen or word 'none'.")
...
def generate_audio(user, places, id):
    ...
    if chosen_id < 0 or chosen_id >= len(places):
        print(f"WARNING: chosen id was {chosen_id}, but there are only {len(places)} places (forced chosen_id=0)")
        chosen_id = 0
...
async def update_user(req: UpdateRequest, authorization: Annotated[str | None, Header()] = None):
    ...
    if PRINT_OUTPUTS:
        print(f"user log (token: {token}, prevent: {req.prevent}, lat: {req.lat}, lon: {req.lon})")
    if token in users:
        user = users[token]
        user.update([req.lat, req.lon])
        if user.do_api_request() and req.prevent == 0:
            if PRINT_OUTPUTS:
                print("   ------------ BEGINNING CONTENT AND AUDIO GENERATION ------------")
