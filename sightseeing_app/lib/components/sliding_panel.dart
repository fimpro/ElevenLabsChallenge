  ...
  void initState() {
    ...
    _ac = new AnimationController(
      ...
      ..addListener(() {
        ...
        var newCollapsedClickable = _ac.value < 0.5;
        if (collapsedClickable != newCollapsedClickable) {
          setState(() {
            collapsedClickable = newCollapsedClickable;
          });
        }
      });
            ...
            builder: (context, child) {
              return Container(
                ...
                _ac.value * (widget.maxHeight - widget.minHeight) +
                    widget.minHeight,
                margin: EdgeInsets.fromLTRB((widget.margin as EdgeInsets).left * (1-_ac.value), (widget.margin as EdgeInsets).top * (1-_ac.value), (widget.margin as EdgeInsets).right * (1-_ac.value), (widget.margin as EdgeInsets).bottom * (1-_ac.value)),
                padding: widget.padding,
                decoration: widget.renderPanelSheet
                    ? BoxDecoration(
                  border: widget.border,
                  borderRadius: BorderRadius.only(topLeft: (widget.borderRadius as BorderRadius).topLeft, topRight: (widget.borderRadius as BorderRadius).topRight, bottomLeft: (widget.borderRadius as BorderRadius).bottomLeft * (1-_ac.value), bottomRight: (widget.borderRadius as BorderRadius).bottomRight * (1-_ac.value)),
                  boxShadow: widget.boxShadow,
                  color: widget.color,
                )
                    : null,
                child: ClipRRect(borderRadius: widget.borderRadius ?? BorderRadius.zero, child: Padding(
                  padding: EdgeInsets.fromLTRB((widget.margin as EdgeInsets).left * _ac.value, (widget.margin as EdgeInsets).top * _ac.value, (widget.margin as EdgeInsets).right * _ac.value,(widget.margin as EdgeInsets).bottom * _ac.value),
                  child: child,
                )),
              );
            },
